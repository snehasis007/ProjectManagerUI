...
export class TaskViewComponent implements OnInit {
  ...
  errorMsg: String;
  constructor(private taskService: ProjectService, private modalService: BsModalService) { }
  ...
  selectProjectAndRelatedTasks(i: number): void {
    this.projectId = this.filteredProjects[i].id;
    this.filteredTasks = this.taskList;
    this.setRelatedTasks(this.projectId);
    this.closeProjectModal();
  }
  setRelatedTasks(projectId: string): void {
    this.filteredTasks = this.filteredTasks.filter(task => task.project.id === projectId);        
  }
